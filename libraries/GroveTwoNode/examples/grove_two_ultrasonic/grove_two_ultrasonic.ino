...
// Change measurement frequncy to 60ms from 30ms.
...
void setup()
{
 ...
 //   nrgSave.begin(GROVE_RX_PIN_NUM, dummy, CHANGE); // The pin need pull up by a resistance
  ...
  StartMillis = millis();
    ...
    wwdg.begin();
    pinMode(ULTRASONIC_SIGNAL_OUT_PIN, INPUT);
...
void ultrasonicMeasureStartType(void)
{
    ...
    uint32_t distance = 0;
 digitalWrite(ULTRASONIC_TX_EN_PIN,HIGH);
 delayMicroseconds(10);
 digitalWrite(ULTRASONIC_TX_EN_PIN,LOW); 
 distance  = pulseIn(ULTRASONIC_SIGNAL_OUT_PIN,HIGH);             //计数接收高电平时间
 distance  = distance*340/2/1840;           //计算距离 1：声速：340M/S  2：实际距离为1/2声速距离 3：计数时钟为1US//温补公式：c=(331.45+0.61t/℃)m•s-1 (其中331.45是在0度）
// Serial.print("距离: ");
   if ((10<=distance)&&(6000>=distance))       //1CM-6M之间数值显示 
    {
  curDistData = distance;  
    }
   else 
    {
   curDistData = 0xffff;                //无效数值数值显示 - - - - 
    }
 //Serial.println();    
 delay(50);                                  //单次测离完成后加30mS的延时再进行下次测量。防止近距离测量时，测量到上次余波，导致测量不准确。
