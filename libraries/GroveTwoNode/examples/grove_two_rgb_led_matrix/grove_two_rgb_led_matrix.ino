...
#define LINE_EN_BIT             0x10
// pins for getting screen number
#define SCREEN_NUM_PIN_LOW      PA6
#define SCREEN_NUM_PIN_HIGH     PB1
...
#define DEVICE_I2C_ADDRESS          0x60
...
#define CONTINUE_PACKET_TIMEOUT     2000

uint8_t base_address = DEVICE_I2C_ADDRESS;
uint8_t offset_address = 0;
uint16_t device_i2c_address = base_address + offset_address;
...
bool testFlag = false;
char *versions = "V22";
uint16_t NodeVersion = 0x6111;
/* Update for bug FLASH, increase delay time to 20 us from 10 us in latchData, 
...
void setup()
{
    ...
    *((uint8_t *)(Flash.blockdata)+I2C_CUR_ADDR_FLASH_LOC) = current_base_address;
    ...
    // base_address + screen number 0-15
    getScreenNumber();
    device_i2c_address = base_address + offset_address;
    ...
    numberToStringList((int16_t)offset_address+1);
    // set color
    display_color = 0;
    // set time
    display_forever_flag = false;

    display_total_time = 1000;
    if (display_list_length < 2) {
...
void displayControlThread(void)
{
    ...
    switch(display_type){
        ...
        case DISPLAY_MUTLI_CHARACTERS: //display_list[]
            if (display_list_position < display_list_length)
            {
                 if ((millis() - display_first_time) >= CONTINUE_PACKET_TIMEOUT)
                 {// Lost packet, timeout.
                     display_type = DISPLAY_NOTHING;
...
void getScreenNumber()
{
    ...
    digitalWrite(LINE_SELECT_EN_PIN, HIGH);
    delay(100);
    uint16_t screen_num_pin_low  =  analogRead(SCREEN_NUM_PIN_LOW);
    uint16_t screen_num_pin_high =  analogRead(SCREEN_NUM_PIN_HIGH);
    
    if (screen_num_pin_low > 900)      offset_address+=0;
    else if (screen_num_pin_low > 600) offset_address+=2;
    else if (screen_num_pin_low > 500) offset_address+=1;
    else if (screen_num_pin_low > 350) offset_address+=3;
    else offset_address+=50;
    if (screen_num_pin_high > 900)      offset_address+=0;
    else if (screen_num_pin_high > 600) offset_address+=8;
    else if (screen_num_pin_high > 500) offset_address+=4;
    else if (screen_num_pin_high > 350) offset_address+=12;
    else offset_address+=50;

    if (offset_address > 15) offset_address = 0;
