...
/***************************************************************
 Board defines
 ***************************************************************/
#define PIR_SIGNAL_PIN_NUM    PA5

#define PIR_GET_DATA_TIMUOUT    3000

uint32_t pirPreviousMillis = 0;
bool pirFlag = false;

/***************************************************************
 Communication defines
 ***************************************************************/
#define DEVICE_I2C_ADDRESS		0x09
#define DEVICE_VID				0x2886
#define DEVICE_PID				0x0007
...
void setup()
{
    ...
    attachInterrupt(PIR_SIGNAL_PIN_NUM, motionEvent, CHANGE, INPUT);
...
void loop()
{
    ...
    if(pirFlag)
    {
        uint32_t pirCurrentMillis = millis();
        if(pirCurrentMillis - pirPreviousMillis >= PIR_GET_DATA_TIMUOUT)
        {
            pirPreviousMillis = pirCurrentMillis;
            
            if(digitalRead(PIR_SIGNAL_PIN_NUM))packet_01_data.data.deviceEvent = true;
            else packet_01_data.data.deviceEvent = false;
#ifdef BLE_SUPPORT
	...
	if (Core_mode == 0)
	{// ATR
		if(millis() - StartMillis >= (I2C_CMD_SYS_READY_TIME+deviceI2CAddress*2))
		{
#if 1
...
void motionEvent()
{
    ...
    if(digitalRead(PIR_SIGNAL_PIN_NUM))
    {
        packet_01_data.data.deviceEvent = true;
        pirPreviousMillis = millis();
        pirFlag = true;
    }
    else
    {
        packet_01_data.data.deviceEvent = false;
        pirFlag = false;
    }
}
