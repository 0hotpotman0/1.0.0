...
#define TM1637_CLK      PA5
#define TM1637_DIO      PA6
...
bool pointFlag[4] = {false, false, false, false};
...
void loop()
{
	...
	if(ledDisplayTimes <= 20)
	{
		uint32_t ledFlashCurrentMillis = millis();
		
		if(ledFlashCurrentMillis - ledDisplayPreviousMillis >= 100)
		{
			if(ledDisplayTimes == 20)
			{
				clearDisplay();
			...
			{
				display(3, ledDisplayTimes/2);
				display(2, ledDisplayTimes/2);
				display(1, ledDisplayTimes/2);
				display(0, ledDisplayTimes/2);
			}
...
void writeByte(uint8_t data)
{
    ...
    {
        digitalWrite(TM1637_CLK, LOW);
        if(data & 0x01)digitalWrite(TM1637_DIO, HIGH);//LSB first
...
void point(int8_t bitAddr, bool point)
{
    if(bitAddr >= 4)return;
    pointFlag[bitAddr] = point;
}
...
{    
    for(uint8_t i = 0; i < 4; i ++)
    {
        ...
        {
            if(pointFlag[i])dispData[i] = tubeTab[dispData[i]] + 0x80;
            else dispData[i] = tubeTab[dispData[i]];
        }
    }
}

uint8_t coding(int8_t bitAddr, uint8_t dispData)
{    
    if(dispData == 0x7f) dispData = 0x00;
    else if (dispData == 0x40) dispData = 0x40;
    else 
    {
        if(pointFlag[bitAddr])dispData = tubeTab[dispData] + 0x80;
        else dispData = tubeTab[dispData];
    }
