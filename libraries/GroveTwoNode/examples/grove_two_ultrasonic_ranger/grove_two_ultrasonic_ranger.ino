...
uint8_t waveNum = 0, *waveData = (uint8_t *)waveCountTimer;
...
void loop()
{
	...
	if(Core_mode == CORE_BLE_MODE)
	{
		...
		if (LightRawData)
		{
			if (((CurrentMillis - RawPreviousMillis) >= RawDelayMillis) && (waveNum>2))
			{
				InstructionOption.raw_data.Header.type		= I2C_CMD_GET_RAW_DATA;
				InstructionOption.raw_data.Header.Address	= deviceI2CAddress;
				InstructionOption.raw_data.Header.Option	= waveNum;
				Value = (waveData[((waveNum-1)*2)]+waveData[((waveNum-1)*2)+1]*256)*10 - (waveNum-1)*25;
