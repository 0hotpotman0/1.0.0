...
void FlashClass::writeAll(void)
{
	...
	{
		FLASH_ProgramWord(BANK1_WRITE_START_ADDR + (i << 2), blockdata[i]);
	}
	FLASH_Lock();
}
...
void FlashClass::write32(uint16_t address, uint32_t data)
{
	...
	uint32_t currentValue = *(__IO uint32_t *)(BANK1_WRITE_START_ADDR + (address << 2));
	...
	if(currentValue != 0xFFFFFFFF)
	{
		for(int i = 0; i < (FLASH_SIZE >> 2); i++)
		{
			blockdata[i] = *(__IO uint32_t *)(BANK1_WRITE_START_ADDR + (i << 2));
		}
	...
	else
	{
		FLASH_Unlock();
		FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);
		FLASH_ProgramWord(BANK1_WRITE_START_ADDR + (address << 2), data);
		FLASH_Lock();
	}
}
...
void FlashClass::write16(uint16_t address, uint16_t data)
{
	...
	uint16_t currentValue = *(__IO uint16_t *)(BANK1_WRITE_START_ADDR + (address << 1));
	if(currentValue == data)return;
	...
	else
	{
		FLASH_Unlock();
		FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);
		FLASH_ProgramHalfWord(BANK1_WRITE_START_ADDR + (address << 1), data);
		FLASH_Lock();
	}
}
...
void FlashClass::write8(uint16_t address, uint8_t data)
{
	...
	uint8_t currentValue = *(__IO uint8_t *)(BANK1_WRITE_START_ADDR + address);
	if(currentValue == data)return;
...
uint32_t FlashClass::read32(uint16_t addr)
{
	...
	data = *(__IO uint32_t *)(BANK1_WRITE_START_ADDR + (addr << 2));
...
uint16_t FlashClass::read16(uint16_t addr)
{
	...
	data = *(__IO uint16_t *)(BANK1_WRITE_START_ADDR + (addr << 1));
...
uint8_t FlashClass::read8(uint16_t addr)
{
	...
	if(addr < FLASH_SIZE)
	data = *(__IO uint8_t *)(BANK1_WRITE_START_ADDR + addr);
