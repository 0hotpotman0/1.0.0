...
uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop) {
	...
	_millis = millis();
	while(I2C_GetFlagStatus(twi, I2C_FLAG_STOP_DET) != RESET)   //I2C_FLAG_BUSY
	{
	...
	{
		_millis = millis();
		while(I2C_GetFlagStatus(twi, I2C_FLAG_RX_DONE) == RESET)
		{
			if(EVENT_TIMEOUT < (millis() - _millis)) return 0;
		}
...
int TwoWire::MasterGPIOTransmission(uint8_t *send, uint8_t sizeofsend)
{
	...
	while((I2C_GetITStatus(twi, I2C_FLAG_STOP_DET) == SET))
	{
		WIRE_I2C_WAIT();
	}
#ifdef I2C_BUS_CHECK
...
{
	begin();
	beginTransmission((uint8_t) (MAINBOARD_BLE_I2C_ADDRESS<<1));
	write(send, sizeofsend);
	endTransmission();
}	
#endif
//
...
{
	if (I2C_GetITStatus(twi, I2C_IT_START_DET) == SET)  //I2C_ISR_BERR
	{
		reset();
	}
	if (I2C_GetITStatus(twi, I2C_IT_ACTIVITY) == SET)  //I2C_IT_ADDR
	{
		I2C_ITConfig(twi, I2C_IT_ACTIVITY | I2C_IT_STOP_DET, ENABLE);  //I2C_IT_RXI | I2C_IT_TXI | I2C_IT_STOPI
		srvBufferLength = 0;
		srvBufferIndex = 0;
		// if (twi->ISR & (1 << 16))
		// {
		...
		I2C_ClearITPendingBit(twi, I2C_IT_ACTIVITY);
	}
	if (I2C_GetITStatus(twi, I2C_IT_ACTIVITY) == SET)
	{
	...
	// }
	if (I2C_GetITStatus(twi, I2C_IT_STOP_DET) == SET)
	{
		...
		I2C_ClearITPendingBit(twi, I2C_IT_STOP_DET);
		I2C_ITConfig(twi, I2C_IT_ACTIVITY| I2C_IT_STOP_DET, DISABLE);
		// I2C_ITConfig(twi, I2C_IT_ACTIVITY, ENABLE);
	}
}
