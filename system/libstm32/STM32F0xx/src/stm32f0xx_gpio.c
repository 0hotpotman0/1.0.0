...
{
  uint32_t pinpos = 0x00, pos = 0x00 , currentpin = 0x00, regval = 0;
  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)
  {
    ...
    if (currentpin == pos)
    {
      if ((GPIO_InitStruct->GPIO_Mode == GPIO_Mode_OUT) || (GPIO_InitStruct->GPIO_Mode == GPIO_Mode_AF))
      {
        ...
        regval = GPIOx->OSPEEDR & (~(GPIO_OSPEEDER_OSPEEDR0 << (pinpos * 2)));
        GPIOx->OSPEEDR = regval | ((uint32_t)(GPIO_InitStruct->GPIO_Speed) << (pinpos * 2));

        /* Check Output mode parameters */
        assert_param(IS_GPIO_OTYPE(GPIO_InitStruct->GPIO_OType));

        /* Output mode configuration */
        regval = GPIOx->OTYPER & (~((GPIO_OTYPER_OT_0) << ((uint16_t)pinpos)));
        GPIOx->OTYPER = regval | ((uint16_t)(((uint16_t)GPIO_InitStruct->GPIO_OType) << ((uint16_t)pinpos)));
      }

      regval = GPIOx->MODER  & (~(GPIO_MODER_MODER0 << (pinpos * 2)));

      GPIOx->MODER = regval | ((((uint32_t)GPIO_InitStruct->GPIO_Mode) << (pinpos * 2)));

      /* Pull-up Pull down resistor configuration */
      regval = GPIOx->PUPDR & (~(GPIO_PUPDR_PUPDR0 << ((uint16_t)pinpos * 2)));
      GPIOx->PUPDR = regval | (((uint32_t)GPIO_InitStruct->GPIO_PuPd) << (pinpos * 2));
    }
  }
}
